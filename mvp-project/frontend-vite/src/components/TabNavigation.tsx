...
export function TabNavigation({ onTabChange }: TabNavigationProps) {
  ...
  return (
    <>
      {/* Мобильная навигация */}
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex md:hidden justify-around items-center h-16 z-30">
        <button
      ...
      <div className="hidden md:flex flex-col space-y-2 fixed left-4 top-1/2 transform -translate-y-1/2 bg-white p-2 rounded-lg shadow-lg">
        <button
          onClick={() => handleTabChange("overview")}
          className={`p-2 rounded-lg hover:bg-gray-100 ${activeTab === "overview" ? "text-primary bg-gray-100" : "text-gray-500"
            }`}
          title="Обзор"
        >
          <Home size={24} />
        ...
        <button
          onClick={() => handleTabChange("transactions")}
          className={`p-2 rounded-lg hover:bg-gray-100 ${activeTab === "transactions" ? "text-primary bg-gray-100" : "text-gray-500"
            }`}
          title="Транзакции"
        >
          <List size={24} />
        ...
        <button
          onClick={() => handleTabChange("chart")}
          className={`p-2 rounded-lg hover:bg-gray-100 ${activeTab === "chart" ? "text-primary bg-gray-100" : "text-gray-500"
            }`}
          title="График"
        >
          <BarChart size={24} />
        ...
        <button
          onClick={() => handleTabChange("settings")}
          className={`p-2 rounded-lg hover:bg-gray-100 ${activeTab === "settings" ? "text-primary bg-gray-100" : "text-gray-500"
            }`}
          title="Настройки"
        >
          <Settings size={24} />
